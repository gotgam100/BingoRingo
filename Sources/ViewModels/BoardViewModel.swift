...
final class BoardViewModel: ObservableObject {
    ...
    private func createDefaultBoard() {
        ...
        for _ in 0..<size {
            var row: [BingoCell] = []
            for _ in 0..<size {
                row.append(BingoCell(title: placeholders[idx % placeholders.count]))
                idx += 1
            }
            cells.append(row)
        }
    ...
    private func detectCompletedLines(board: BingoBoard) {
        ...
        for i in 0..<size {
            if board.cells[i].allSatisfy({ $0.isCompleted(for: board.memberIDs) }) {
                lines.append(BingoReward(lineIndex: i, lineType: .row, description: ""))
            }
            if (0..<size).allSatisfy({ board.cells[$0][i].isCompleted(for: board.memberIDs) }) {
                lines.append(BingoReward(lineIndex: i, lineType: .column, description: ""))
            }
        }
        if (0..<size).allSatisfy({ board.cells[$0][$0].isCompleted(for: board.memberIDs) }) {
            lines.append(BingoReward(lineIndex: 0, lineType: .diagonal, description: ""))
        }
        if (0..<size).allSatisfy({ board.cells[$0][size - 1 - $0].isCompleted(for: board.memberIDs) }) {
